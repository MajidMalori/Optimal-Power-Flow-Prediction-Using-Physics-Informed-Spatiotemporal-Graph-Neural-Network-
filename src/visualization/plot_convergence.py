...
def plot_convergence_story(data_dir: str, case_name: str, output_path: str, config=None) -> str:
    ...
    if not audits:
        print(f"No convergence data found for {case_name} in {data_dir}")
        return None
    ...
    for ren_frac, audit in audits.items():
        ...
        total = stats.get('total_timesteps', 1)
        ...
        val_stats = stats.get('validation_stats', {})
        trip_count = val_stats.get('generator_trips', max(0, successful - known_sum))
