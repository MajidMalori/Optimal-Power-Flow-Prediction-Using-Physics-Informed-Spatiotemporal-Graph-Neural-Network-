...
from src.visualization.plot_warmstart import plot_warmstart_metrics
    ...
    for case in cases:
        ...
        for model_name in models_to_run:
            ...
            def model_run_fn(state, *args_fn, **kwargs_fn):
                ...
                return run_all_methods_for_state(state, *args_fn, pred_vm=pred_vm, pred_va=pred_va, **kwargs_fn)

            print(f"\n--- Running Speed Benchmark: {case.upper()} ({model_name}) ---")
            records, summary = run_speed_benchmark(states, model_run_fn)
            
            # Save single model speed logs
            out_dir = os.path.join(PROJECT_ROOT, "reports", "warmstart", "speed", case, model_name)
            ...
            df_model.to_csv(os.path.join(out_dir, f"{case}_speed_records.csv"), index=False)
        ...
        if len(models_to_run) > 1:
            global_out_dir = os.path.join(PROJECT_ROOT, "reports", "warmstart", "speed", case, "all_comparison")
            os.makedirs(global_out_dir, exist_ok=True)

            df_all = pd.DataFrame(all_records)
            df_all.to_csv(os.path.join(global_out_dir, f"{case}_all_models_speed_records.csv"), index=False)

            # Map to format expected by plot_warmstart_metrics
            # df columns: Model, InitMethod, Time_ms, Iterations, Success
            METHOD_MAP = {
            ...
            df_plot = pd.DataFrame({
                "Model": df_all["model"],
                "InitMethod": df_all["method"].map(METHOD_MAP),
                "Time_ms": df_all["time_ms"],
                "Iterations": df_all["iterations"],
                "Success": df_all["converged"]
            })

            plot_warmstart_metrics(df_plot, case, global_out_dir)
            print(f"[SUCCESS] Global speed comparison plots generated to: {global_out_dir}")
