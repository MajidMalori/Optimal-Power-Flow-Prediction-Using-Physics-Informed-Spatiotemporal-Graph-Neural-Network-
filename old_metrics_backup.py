...
class PowerSystemLoss(nn.Module):
    ...
    def __init__(self, config: object, normalizer, is_gcn: bool = False):
        ...
        # Physics losses: Kendall-style learnable weights (Kendall et al., CVPR 2018)
        # 
        # Smart Initialization (FIXED):
        # - log_sigma_power = 0.0 → sigma = 1.0 → weight = 0.5 (power balance is complex, start neutral)
        # - log_sigma_voltage = -2.0 → sigma = 0.135 → weight = 27.4 (voltage is critical, start with high weight)
        # 
        # This encourages the model to respect voltage constraints early in training,
        # preventing the "cheating" behavior where voltage violations are accepted.
        # NOTE: If sigma_voltage becomes too large (e.g., > 7), weight becomes < 0.01, effectively disabling voltage penalty
        # The warning in forward() will alert you if this happens.
        self.log_sigma_power = nn.Parameter(torch.tensor(0.0))  # Physics loss: Kendall-style (initialized to 1.0, weight=0.5)
        self.log_sigma_voltage = nn.Parameter(torch.tensor(-2.0))  # Physics loss: Kendall-style (initialized to 0.135, weight=27.4)
    ...
    def forward(self, 
        ...
        # Units: power_penalty is in (p.u.)², voltage_penalty is in (p.u.)²
        #        sigma² is dimensionless, so weighted losses are in (p.u.)² (same as penalties)
        weighted_power_loss = (1.0 / (2.0 * sigma_power ** 2)) * power_penalty  # Weighted power loss (p.u.)²
        weighted_voltage_loss = (1.0 / (2.0 * sigma_voltage ** 2)) * voltage_penalty  # Weighted voltage loss (p.u.)²
        
        # DIAGNOSTIC: Warn if voltage loss weight becomes too small (allows model to "cheat")
        # This happens when sigma_voltage becomes large, making voltage violations "cheaper" than fixing them
        effective_lambda_v = 1.0 / (2.0 * sigma_voltage ** 2)  # Effective weight on voltage penalty
        if effective_lambda_v.item() < 0.01:  # If weight < 0.01, voltage penalty is essentially disabled
            import warnings
            warnings.warn(
                f"[VOLTAGE PENALTY WARNING] Effective voltage loss weight is very small: {effective_lambda_v.item():.6f}. "
                f"sigma_voltage = {sigma_voltage.item():.4f}. "
                f"The model may be accepting voltage violations because they're 'cheaper' than learning correct physics. "
                f"This can lead to physically impossible predictions (e.g., 0.2 p.u. voltages). "
                f"Consider: (1) Better initialization of log_sigma_voltage, (2) Architecture improvements, "
                f"(3) More training data, or (4) Harder voltage constraints.",
                UserWarning
            )
        
        # Regularization: log(sigma) terms for physics losses (Kendall-style)
        # This prevents sigmas from becoming infinite (which would disable the loss terms)
        # Units: log(sigma) is dimensionless (log of dimensionless parameter)
    ...
    def _compute_power_balance_violation(self, predicted_voltages, measured_power, ybus_batch, bus_types: torch.Tensor, squared=True, debug=False):
        ...
        if squared:
            ...
            return torch.mean(mismatch_squared, dim=-1)  # [batch] - MSE (p.u.)²
        else:
    ...
    def _compute_voltage_limit_violation(self, state: torch.Tensor, bus_types: torch.Tensor, squared=True) -> torch.Tensor:
        ...
        violation_all = v_below**2 + v_above**2  # [batch, buses] - Squared violations (p.u.)²
        
        # Mask out non-PQ buses (set their violations to zero)
        pq_mask = (bus_types == 0)  # [batch, buses] - PQ buses (boolean mask)
        violation_all = violation_all * pq_mask.float()  # Zero out non-PQ buses (p.u.)²
        
        if squared:
            # Return MSE: mean of squared violations across buses
            # Units: (p.u.)² - per-unit squared (for training loss)
            return torch.mean(violation_all, dim=-1)  # [batch] - MSE (p.u.)²
        else:
    ...
    def _compute_mean_power_flow_pu(self, voltages: torch.Tensor, measurements: torch.Tensor, Ybus: torch.Tensor, bus_types: torch.Tensor, epsilon: float = 1e-9) -> torch.Tensor:
        ...
        # Physical interpretation: 0.5 p.u. = 50% of system base power flowing through network
        # NOTE: These are the actual physical flow magnitudes. We don't normalize by load or clamp.
        # Large values indicate high power flows, which is physically meaningful information.
        return mean_flow_magnitude_per_bus  # [batch_size] - mean flow magnitude per bus in per-unit
