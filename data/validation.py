...
def validate_power_flow_outputs(net: pp.pandapowerNet, convergence_stats: dict, case_name: str = None) -> tuple[bool, str, dict]:
    ...
    v_min, v_max = (0.70, 1.20) if case_name and "57" in case_name else (0.85, 1.15)
    ...
    if not net.line.empty:
        ...
        if np.max(diffs) > np.pi/2: return False, "Angle difference > 90 deg (garbage)", flags
        if np.max(diffs) > np.deg2rad(45): flags['angle_violation'] = True
...
def apply_curtailment_with_retry(net: pp.pandapowerNet, base_renewable_p_mw: dict, 
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            valid, reason, flags = validate_power_flow_outputs(net, convergence_stats or {}, case_name)
            if valid:
...
def hard_reset_system(net: pp.pandapowerNet, base_load_p: np.ndarray, base_load_q: np.ndarray,
    ...
    try:
        ...
        valid, _, _ = validate_power_flow_outputs(net, convergence_stats or {}, case_name)
        if valid: return True, None
        
        return trip_renewable_generators(net, convergence_stats, case_name), None
