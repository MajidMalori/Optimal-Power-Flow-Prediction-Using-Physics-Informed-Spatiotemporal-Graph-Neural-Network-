...
def plot_physics_health(config, case_name: str, output_path: str) -> str:
    ...
    Shows:
    1. Voltage magnitude distribution (all buses, all samples) - FULL VIEW including 0.0s
    2. Per-bus voltage statistics
    """
    ...
    if case_num == 33:
        v_min, v_max = 0.90, 1.10  # Distribution system
        v_emergency_min, v_emergency_max = 0.85, 1.15
    else:
        v_min, v_max = 0.90, 1.10  # Transmission system  
        v_emergency_min, v_emergency_max = 0.85, 1.15
    ...
    # Use bins covering the full range
